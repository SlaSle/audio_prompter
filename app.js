...
function cueFirstLine() {
  const firstVerse = [...els.prompter.querySelectorAll(".song-line.section")]
    .find((line) => line.dataset.section?.startsWith("zwrotka"));
  const firstLine = firstVerse
    ? getPlayableLines().find((line) => Number(line.dataset.index) > Number(firstVerse.dataset.index))
    : getPlayableLines()[0];
  if (!firstLine) return;
...
function advancePlayback(now = performance.now()) {
  ...
  if (state.midiPedalMode) {
    ...
    els.playbackStatus.textContent = `${lineProgress >= 1 ? "czeka na pedal" : `${lineProgress.toFixed(2)} linii`}`;
    window.scrollTo(0, 0);
...
function handleMidiMessage(event) {
  ...
  if (decoded.type === "sustain") {
    ...
    const isDown = getSustainIsDown(decoded.value);
...
function armMidiStart() {
  ...
  if (state.midiWaitForStart) {
    ...
    state.midiSustainDown = false;
    state.midiSustainHighIsDown = null;
    if (!state.startedSong) cueFirstLine();
    els.playbackStatus.textContent = "czeka na MIDI";
    updateMidiStatus("Czekam na sygnal z Casio. Okno zamykam, a linie zmieniasz puszczeniem pedalu sustain.");
    closeMidiDialog();
  } else {
