...
function advanceAutoMap(elapsed) {
  ...
  const rate = getAutoMapRate(map);
  const mapElapsedMs = elapsed * 1000 * rate;

  while (
    state.autoMapNextTransition < map.transitions.length
    && mapElapsedMs >= map.transitions[state.autoMapNextTransition].atMs
  ) {
  ...
  if (next) {
    const remaining = Math.max(0, (next.atMs - mapElapsedMs) / rate / 1000);
    els.playbackStatus.textContent = `auto ${rate.toFixed(2)}x: ${state.autoMapNextTransition}/${map.transitions.length}  ${remaining.toFixed(1)}s`;
  } else {
    els.playbackStatus.textContent = `auto ${rate.toFixed(2)}x: ${map.transitions.length}/${map.transitions.length}`;
  }
}

function getAutoMapRate(map) {
  const learnedBpm = clamp(Number(map.learnedBpm) || Number(els.bpmInput.value) || 96, 30, 240);
  const currentBpm = clamp(Number(els.bpmInput.value) || learnedBpm, 30, 240);
  const fineSpeed = clamp(Number(els.speedNumberInput.value) || 1, 0.25, 2.5);
  return (currentBpm / learnedBpm) * fineSpeed;
}
...
function startAutoMapPlayback() {
  ...
  updatePlayButton();
  const rate = getAutoMapRate(map);
  els.playbackStatus.textContent = `auto ${rate.toFixed(2)}x: 0/${map.transitions.length}`;
...
function finishLearning() {
  ...
  const map = {
    ...
    learnedBpm: clamp(Number(els.bpmInput.value) || parsed.meta.bpm, 30, 240),
    learnedRhythm: els.rhythmSelect.value,
